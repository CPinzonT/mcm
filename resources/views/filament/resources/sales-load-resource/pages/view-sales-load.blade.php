...
@php
    ...
    $previewErrors = collect($record->error_log ?? [])->take(5);
@endphp

<div class="ops-shell">
    <section class="ops-hero ops-hero--compact">
        <div>
            <div class="ops-hero-kicker">Detalle de ventas</div>
            <div class="ops-title">{{ $record->reference }}</div>
            <p class="ops-subtitle">
                {{ $record->original_filename ?: basename($record->path) }} · {{ $record->period_key ?: 'Sin periodo' }}
            </p>
        </div>
    ...
    <section class="ops-metric-strip" aria-label="Resumen de la carga de ventas">
        ...
        <article class="ops-metric">
            ...
            <div class="ops-metric-value">{{ number_format($record->processed_rows) }}</div>
            <div class="ops-metric-sub">Insertados en ventas</div>
        </article>
        <article class="ops-metric">
            <div class="ops-metric-label">Valor</div>
            <div class="ops-metric-value">${{ number_format($record->total_amount, 0, ',', '.') }}</div>
            <div class="ops-metric-sub">Total de la carga</div>
        </article>
    </section>
    ...
    <section class="ops-load-layout">
        <article class="ops-panel">
            ...
            <p class="ops-section-copy">Datos fuente, origen Azure y auditoria principal.</p>
            ...
            <div class="ops-meta-grid" style="margin-top: 1rem;">
                ...
                <div><div class="ops-meta-label">Origen Azure</div><div class="ops-meta-value">{{ $record->source_url ?: 'Carga manual' }}</div></div>
        ...
        <aside class="ops-side-stack">
            <div class="ops-panel ops-panel-soft">
                ...
                <div class="ops-compact-list">
                    ...
                    @if($record->notes)
                        <div><strong>Notas</strong><span>{{ $record->notes }}</span></div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="ops-preview-list">
                        @foreach($previewErrors as $error)
                            <div class="ops-preview-item">
                                <strong>{{ ($error['row_number'] ?? null) ? 'Fila ' . $error['row_number'] : 'Validacion general' }}</strong>
