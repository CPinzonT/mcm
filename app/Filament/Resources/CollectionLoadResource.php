...
use App\Filament\RelationManagers\CollectionDetailsRelationManager;
use App\Filament\RelationManagers\LoadAuditsRelationManager;
use App\Filament\RelationManagers\LoadErrorsRelationManager;
use App\Filament\Resources\CollectionLoadResource\Pages;
...
class CollectionLoadResource extends Resource
{
    protected static ?string $model = CollectionLoad::class;
    ...
    protected static ?string $modelLabel = 'Carga de recaudos';

    protected static ?string $pluralModelLabel = 'Cargas de recaudos';
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('original_filename')
                    ->label('Archivo')
                    ->limit(36)
                    ->tooltip(fn (CollectionLoad $record): ?string => $record->original_filename)
            ...
            ->actions([
                ...
                Action::make('errores')
                    ...
                    ->url(fn (CollectionLoad $record): string => route('admin.loads.collection.errors', $record))
                    ->openUrlInNewTab()
                    ->visible(fn (CollectionLoad $record): bool => $record->error_rows > 0),
                Action::make('anular')
                    ->label('Anular')
                    ->icon('heroicon-o-no-symbol')
                    ->color('danger')
                    ->requiresConfirmation()
                    ->visible(fn (CollectionLoad $record): bool => $record->status === 'completed' && auth()->user()?->hasRole('admin'))
                    ->action(function (CollectionLoad $record): void {
                        try {
                            app(PeriodControlService::class)->cancelCollectionLoad(
                                load: $record,
                                user: auth()->user(),
                            ...
                            app(LoadAuditService::class)->record(
                                ...
                                'collection',
                                'cancelled',
                                'Carga de recaudos anulada desde historial.',
                                auth()->user(),
                            );
    ...
    {
        return [
            CollectionDetailsRelationManager::class,
    ...
    {
        return [
            'index' => Pages\ListCollectionLoads::route('/'),
            'view' => Pages\ViewCollectionLoad::route('/{record}'),
        ];
    }
}
